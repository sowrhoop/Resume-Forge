    ...
    <section id="layout" className="grid gap-y-6">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-x-4">
          <SectionIcon id="layout" size={18} name={`Layout`} />
          <h2 className="line-clamp-1 text-2xl font-bold lg:text-3xl">{`Layout`}</h2>
        </div>
      ...
      <main className="grid gap-y-4">
            ...
            return (
              <div key={pageIndex} className="rounded border p-3 pb-4">
                <div className="flex items-center justify-between">
                  <p className="mb-3 text-xs font-bold">{`Page ${pageNumber}`}</p>
        ...
        <Button variant="outline" className="ml-auto" onClick={onAddPage}>
          <PlusIcon />
          <span className="ml-2 text-xs lg:text-sm">{`Add New Page`}</span>
        </Button>
      </main>
    </section>
  );
