...
export const OpenAISettings = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-2xl font-bold leading-relaxed tracking-tight">{`OpenAI/Azure OpenAI/Ollama Integration`}</h3>
        <p className="leading-relaxed opacity-75">
          {`You can make use of the OpenAI API, Azure OpenAI, or Ollama to help you generate content, or improve your writing while composing your resume.`}
        </p>
      </div>
      ...
      <Form {...form}>
        <form className="grid gap-6 sm:grid-cols-2" onSubmit={form.handleSubmit(onSubmit)}>
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{`OpenAI/Ollama API Key`}</FormLabel>
                <FormControl>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  {form.watch("isAzure") ? `Azure OpenAI Resource URL` : `Base URL`}
                </FormLabel>
                <FormControl>
                  <Input
                    type="text"
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{form.watch("isAzure") ? `Deployment Name` : `Model`}</FormLabel>
                <FormControl>
            ...
            render={({ field }) => (
              <FormItem className="flex flex-row items-center space-x-3 space-y-0">
                ...
                <div className="space-y-1 leading-none">
                  <FormLabel>{`Use Azure OpenAI`}</FormLabel>
                </div>
                <FormMessage />
              </FormItem>
            )}
